...
function testEnvironmentDetection() {
    ...
    const flagsConsistent = (ENV.IS_PRODUCTION && !ENV.IS_DEVELOPMENT && !ENV.IS_TEST) ||
    ...
    if (flagsConsistent) {
        ...
        console.log(`Production: ${ENV.IS_PRODUCTION}, Development: ${ENV.IS_DEVELOPMENT}, Test: ${ENV.IS_TEST}`);
        process.exit(1);
    }
    // Test IS_DEV_OR_TEST flag
    const devOrTestConsistent = ENV.IS_DEV_OR_TEST === (ENV.IS_DEVELOPMENT || ENV.IS_TEST);
    ...
    for (const testCase of testCases) {
        ...
        if (result === testCase.expected) {
            ...
            console.log(`  ❌ "${testCase.nodeEnv}" → "${result}" (expected "${testCase.expected}")`);
            process.exit(1);
        }
    }
...
//# sourceMappingURL=test-environment-config.js.map