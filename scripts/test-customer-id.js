...
console.log(`${uniqueRapidIds.size === rapidIds.length ? "✅" : "❌"} All unique: ${uniqueRapidIds.size === rapidIds.length}`);
...
console.log(`${uniqueDelayedIds.size === delayedIds.length ? "✅" : "❌"} All unique: ${uniqueDelayedIds.size === delayedIds.length}`);
// Test 3: Simulate concurrent generation (Promise.all)
console.log("\n🚀 Concurrent Generation Test (20 IDs simultaneously):");
const concurrentPromises = Array.from({ length: 20 }, (_, i) => Promise.resolve().then(() => {
    const id = generateUniqueCustomerId();
    console.log(`  ${i + 1}. ${id}`);
    return id;
}));
...
console.log(`${uniqueConcurrentIds.size === concurrentIds.length ? "✅" : "❌"} All unique: ${uniqueConcurrentIds.size === concurrentIds.length}`);
...
console.log(`  Total possible combinations: 16^8 = ${Math.pow(16, 8).toLocaleString()}`);
...
console.log(`  8-character hex: ${Math.pow(16, 8).toLocaleString()} possible values`);
console.log(`  Collision probability: Virtually zero for practical use`);
console.log("\n🎉 Customer ID generation test completed!");
//# sourceMappingURL=test-customer-id.js.map