...
async function testProgressiveDataCreation() {
    ...
    for (let i = 0; i < testCustomers.length; i++) {
        ...
        try {
            ...
            console.log(`  📱 Creating phone record for ${customer.phoneNumber.slice(-4)}...`);
            const phoneRecord = await dataService.createOrUpdatePhoneRecord(customer.phoneNumber);
...
async function testCustomerRetrieval() {
    ...
    for (let i = 0; i < Math.min(3, testCustomers.length); i++) {
        const customer = testCustomers[i];
        try {
            console.log(`  📞 Looking up customer by phone ${customer.phoneNumber.slice(-4)}...`);
            const retrievedCustomer = await dataService.getCustomerByPhone(customer.phoneNumber);
            if (retrievedCustomer) {
                console.log(`  ✅ Found: ${retrievedCustomer.customerId} - ${retrievedCustomer.fullName}`);
            }
            else {
