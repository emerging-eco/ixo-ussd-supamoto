...
class SessionLogger {
    ...
    writeMetadata(sessionId, phoneNumber, serviceCode) {
        const metadata = {
            sessionId,
            phoneNumber,
            serviceCode,
    ...
    writeToLog(message) {
        try {
            ...
            this.originalConsoleError("⚠️  Failed to write to log file:", error instanceof Error ? error.message : String(error));
        }
    }
    /**
...
async function sendRequest(sessionId, text) {
...
function promptForTestGeneration() {
    rl.question("\n💡 Would you like to generate a test from this session? (y/n): ", answer => {
        if (answer.toLowerCase() === "y" || answer.toLowerCase() === "yes") {
            rl.question("Enter a name for this flow (e.g., login-flow): ", flowName => {
                if (flowName && flowName.trim().length > 0) {
                    const logPath = sessionLogger?.getLogFilePath();
                    console.log("\n📝 To generate the test, run:");
                    console.log(`   pnpm generate:test ${logPath} ${flowName.trim()}\n`);
                }
                else {
    ...
    rl.question(prompt, async (input) => {
        ...
        try {
            ...
            if (response.startsWith("END")) {
                ...
                askQuestion("Enter your choice (or 'debug' for session info, 'exit' to quit): ");
            }
        }
        catch (error) {
