...
const mockProgressiveDataService = {
    createOrUpdatePhoneRecord: async (phoneNumber) => {
    ...
    createCustomerRecord: async (phoneId, customerData) => {
...
global.progressiveDataService = mockProgressiveDataService;
...
console.log("After account creation completes, IXO blockchain account creation");
console.log("will continue in the background without blocking the user experience.");
console.log("Check logs/ixo-creation-failures.log for any background processing issues.");
// Cleanup after 30 seconds if demo doesn't complete
setTimeout(() => {
    console.log("\n⏰ Demo timeout - exiting");
    process.exit(0);
}, 30000);
//# sourceMappingURL=accountCreationMachine-demo.js.map
