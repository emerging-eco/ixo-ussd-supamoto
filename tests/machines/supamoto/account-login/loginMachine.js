...
export const INVALID_IDENTIFIER = "Invalid identifier format. Please try again.";
export const CUSTOMER_NOT_FOUND_MSG = "Customer ID or National ID not found. Please check and try again or contact support.";
export const PIN_FIELD_EMPTY_MSG = "Your account needs PIN setup. Please contact support.";
export const INCORRECT_PIN_MSG = (attempt) => {
...
export const MAX_ATTEMPTS_MSG = "Your account was locked after 3 incorrect PIN attempts. Please contact your LG to activate your account.";
export const VERIFYING_MSG = "Verifying identifier...\n1. Continue";
export const VERIFYING_PIN_MSG = "Verifying PIN...\n1. Continue";
export const LOGIN_SUCCESS_MSG = (customerId, customerFullName) => `Welcome, ${customerFullName}!\nLogin successful for Customer ID: ${customerId}.\n1. Continue`;
/**
 ...
 */
const hasMaxAttemptsExceeded = ({ context }) => {
 ...
 */
const customerLookupService = fromPromise(async ({ input }) => {
    logger.info({ identifier: input.customerId.slice(-4) }, "Looking up customer by identifier (customer ID or national ID)");
    const customer = await dataService.getCustomerByIdentifier(input.customerId);
    if (!customer) {
 ...
 */
const pinVerificationService = fromPromise(async ({ input, }) => {
    logger.info({
    ...
    if (!isValid) {
        ...
        if (input.attempts >= 3) {
            logger.warn({ customerId: input.customerId.slice(-4) }, "Max PIN attempts exceeded, clearing PIN");
 ...
 */
const loadCustomerDataService = fromPromise(async ({ input }) => {
    logger.info({ customerId: input.customerId.slice(-4) }, "Loading decrypted customer data from SDK");
    try {
        const customerData = await getDecryptedCustomerData(input.customerId);
        if (!customerData) {
            logger.warn({ customerId: input.customerId.slice(-4) }, "Customer data not found in SDK");
            return null;
        }
        logger.info({
 ...
 */
const verifyCredentialsService = fromPromise(async ({ input, }) => {
    logger.info({
    ...
    const customer = await dataService.getCustomerByIdentifier(input.customerId);
    if (!customer) {
        logger.warn({ identifier: input.customerId.slice(-4) }, "Customer not found by identifier");
        throw new Error("CUSTOMER_NOT_FOUND");
    }
    // Step 2: Check if encrypted PIN exists
    if (!customer.encryptedPin) {
        logger.warn({ customerId: input.customerId.slice(-4) }, "Encrypted PIN field is empty - account needs activation");
        throw new Error("ENCRYPTED_PIN_FIELD_EMPTY");
    }
    // Step 3: Verify PIN
    let isValid = false;
    ...
    if (!isValid) {
        ...
        if (input.attempts >= 3) {
            logger.warn({ customerId: input.customerId.slice(-4) }, "Max PIN attempts exceeded, locking account");
            // Clear the PIN (set to NULL)
            await dataService.clearCustomerPin(input.customerId);
            // Create audit log entry
            await dataService.createAuditLog({
...
export const loginMachine = setup({
    ...
    states: {
        ...
        verifyingCredentials: {
            ...
            invoke: {
                ...
                onError: [
                    {
                        target: "routeToMain",
                        guard: ({ event }) => event.error?.message === "CUSTOMER_NOT_FOUND",
                        actions: [
                            assign({
                    ...
                    {
                        target: "routeToMain",
                        guard: ({ event }) => event.error?.message === "ENCRYPTED_PIN_FIELD_EMPTY",
                        actions: [
                            assign({
                    ...
                    {
                        target: "routeToMain",
                        guard: ({ event }) => event.error?.message === "MAX_ATTEMPTS_EXCEEDED",
                        actions: [
                            assign({
                    ...
                    {
                        target: "pinEntry",
                        guard: ({ event }) => event.error?.message?.startsWith("INCORRECT_PIN:"),
                        actions: [
                            assign(({ event }) => {
        ...
        error: {
            on: {
                INPUT: withNavigation([{ target: "customerIdEntry" }], NavigationPatterns.loginChild),
                ERROR: {
                    actions: [
                        assign(({ event }) => ({
                            error: event.error || "An error occurred",
...
//# sourceMappingURL=loginMachine.js.map
