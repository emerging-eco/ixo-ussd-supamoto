...
const mockProgressiveDataService = {
    ...
    async clearCustomerPin(customerId) {
...
const mockVerifyPin = (inputPin, encryptedPin) => {
    console.log(`🔐 Verifying PIN: ${inputPin} against ${encryptedPin}`);
    // For demo: configurable PIN verification
    return (inputPin === config.DEMO_CONFIG.VALID_PIN &&
        encryptedPin === config.DEMO_CONFIG.ENCRYPTED_PIN_HASH);
};
// Replace the actual imports with mocks
globalThis.progressiveDataService = mockProgressiveDataService;
globalThis.verifyPin = mockVerifyPin;
function createDemoActor(scenario) {
...
async function runDemo() {
    ...
    console.log(`📝 Message after invalid input: ${demo8.getSnapshot().context.message}`);
...
//# sourceMappingURL=loginMachine-demo.js.map
