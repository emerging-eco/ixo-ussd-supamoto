...
export const pinChangeMachine = setup({
    ...
    actors: {
        updatePinService: fromPromise(async ({ input }) => {
            logger.info({ customerId: input.customerId.slice(-4) }, "Updating customer PIN");
            ...
            logger.info({ customerId: input.customerId.slice(-4) }, "PIN updated successfully");
            return { success: true };
        }),
    },
}).createMachine({
            ...
            on: {
                INPUT: [
                    ...
                    },
                    {
                        target: "enterNewPin",
                        actions: assign({
                            message: "Invalid PIN format. Your PIN must be exactly 5 digits (numbers only).\n\nPlease create a new 5-digit PIN:",
...
//# sourceMappingURL=pinChangeMachine.js.map
