...
import { generatePin, sendActivationSMS, sendEligibilityConfirmationSMS, } from "../../../../src/services/sms.js";
const logger = createModuleLogger("customerActivation");
export const CustomerActivationOutput = {
    COMPLETE: "COMPLETE",
    CANCELLED: "CANCELLED",
    ERROR: "ERROR",
    UNDEFINED: "UNDEFINED"
...
const ENTER_PHONE_PROMPT = "Enter customer's phone number (with country code, e.g., +260971234567):";
const ACTIVATION_SMS_SENT = (customerId) => `Activation SMS sent to customer ${customerId}.\nCustomer will receive a temporary PIN to activate their account.\n1. Continue`;
const ENTER_TEMP_PIN_PROMPT = "Welcome! Enter the temporary PIN sent to your phone:";
...
const ELIGIBILITY_QUESTION = "Are you part of a 1,000-day household?\n(Pregnant or with child under 2 years)\n1. Yes\n2. No";
const NOT_ELIGIBLE_MSG = "Thank you for your response. Your information has been recorded.\n1. Continue";
const SUBMITTING_CLAIM_MSG = "Submitting your eligibility claim...\n1. Continue";
const ELIGIBILITY_CONFIRMED = "Congratulations! You are eligible for the bean distribution program. You will receive an SMS confirmation.\n1. Continue";
// Validation guards
const isValidCustomerId = ({ event }) => {
    if (event.type !== "INPUT")
        return false;
    const customerId = event.input.trim();
    return /^C[A-Za-z0-9]{8,}$/.test(customerId);
};
const isValidPhoneNumber = ({ event }) => {
    if (event.type !== "INPUT")
        return false;
...
// Actors
const generateAndSendPinService = fromPromise(async ({ input }) => {
    logger.info({
    ...
    try {
        ...
        await dataService.resetCustomerPin(input.customerId, tempPin, undefined // lgCustomerId not available in this context
        );
        logger.info({ customerId: input.customerId.slice(-4) }, "✅ Temporary PIN set successfully");
        // Send SMS
        logger.info({
            customerId: input.customerId.slice(-4),
            phoneNumber: input.phoneNumber.slice(-4),
        }, "📱 Sending activation SMS");
        const smsResult = await sendActivationSMS(input.phoneNumber, input.customerId, tempPin);
...
const verifyPinService = fromPromise(async ({ input, }) => {
    logger.info({ customerId: input.customerId.slice(-4) }, "🔐 Verifying temporary PIN");
    try {
        // Get customer record
        ...
        const { encryptPin } = await import("../../../../src/utils/encryption.js");
        const encryptedInputPin = encryptPin(input.pin);
        const isValid = encryptedInputPin === customer.encryptedPin;
        if (!isValid) {
            logger.warn({ customerId: input.customerId.slice(-4) }, "❌ Temporary PIN verification failed - invalid PIN");
            throw new Error("INVALID_PIN");
        }
        logger.info({ customerId: input.customerId.slice(-4) }, "✅ Temporary PIN verified successfully");
...
const submitClaimService = fromPromise(async ({ input, }) => {
    logger.info({ customerId: input.customerId.slice(-4) }, "Submitting 1,000-day household claim");
    ...
    logger.warn({ customerId: input.customerId.slice(-4) }, "STUB: Would submit claim to ixo-matrix-supamoto-claims-bot");
    // Simulate claim submission
    const stubClaimId = `claim-${Date.now()}`;
    return { claimId: stubClaimId };
});
const sendConfirmationService = fromPromise(async ({ input }) => {
    logger.info({ phoneNumber: input.phoneNumber.slice(-4), customerId: input.customerId.slice(-4) }, "🎉 Sending eligibility confirmation SMS");
    try {
        ...
        logger.warn({ phoneNumber: input.phoneNumber.slice(-4) }, "⏳ STUB: Would transfer BEAN token via subscriptions-service-supamoto");
        return { sent: true };
    }
    catch (error) {
        logger.error({
...
export const customerActivationMachine = setup({
    ...
    guards: {
        ...
        isExit: ({ event }) => event.type === "INPUT" && event.input.toLowerCase() === "exit",
    },
    actors: {
                ...
                INPUT: [
                    {
                        ...
                        actions: assign({
                            customerId: ({ event }) => event.type === "INPUT" ? event.input.trim() : undefined,
                        }),
                    },
                    {
                        target: "verifyCustomer",
                        actions: assign({
                            message: "Invalid Customer ID format. Please enter a valid Customer ID (e.g., C12345678):",
                        }),
                    },
                ],
                ERROR: {
                    ...
                    actions: assign({
                        ...
                        message: ({ event }) => event.type === "ERROR"
                            ? `Error: ${event.error}\n0. Back`
                            : "An error occurred\n0. Back",
                    }),
                },
                ...
                INPUT: [
                    {
                        ...
                        actions: [
                            assign({
                                customerPhone: ({ event }) => event.type === "INPUT" ? event.input.trim() : undefined,
                            }),
                            ({ context, event }) => {
                                ...
                                if (event.type === "INPUT" && context.customerId) {
                                    ...
                                    (async () => {
                                        try {
                                            ...
                                            const tempPin = generatePin();
                                            // Reset customer PIN with temporary PIN
                                            await dataService.resetCustomerPin(context.customerId, tempPin, undefined // lgCustomerId not available in this context
                                            );
                                            logger.info({ customerId: context.customerId.slice(-4) }, "✅ Temporary PIN set successfully");
                                            // Send activation SMS
                                            const smsResult = await sendActivationSMS(phoneNumber, context.customerId, tempPin);
                                            if (!smsResult.success) {
                                                throw new Error(`SMS delivery failed: ${smsResult.error || "Unknown error"}`);
                                            }
                                            logger.info({
                    ...
                    },
                    {
                        target: "enterPhone",
                        actions: assign({
                            message: "Invalid phone number. Please enter with country code (e.g., +260971234567):",
                        }),
                    },
                ],
                ERROR: {
            ...
            on: {
                INPUT: [
                    ...
                    },
                    {
                        target: "customerEnterPin",
                        actions: assign({
                            message: "Invalid PIN format. Please enter the 6-digit PIN from your SMS:",
                        }),
                    },
                ],
                ERROR: {
...
//# sourceMappingURL=customerActivationMachine.js.map
