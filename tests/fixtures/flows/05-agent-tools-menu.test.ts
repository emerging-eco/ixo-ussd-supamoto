/**
 * Generated Test: 05-agent-tools-menu
 *
 * This test was automatically generated from a recorded USSD session.
 *
 * Session Details:
 * - Flow: 05-agent-tools-menu
 * - Session ID: rec-05-agent-tools-menu-1774608364940-13
 * - Phone: +260971230001
 * - Service Code: *2233#
 * - Recorded: 2026-03-27T10:46:09.664Z
 * - Turns: 8
 *
 ...
 *    pnpm test tests/flows/05-agent-tools-menu.test.ts  # Won't work
 *
 * Prerequisites:
 ...
 * 3. Run: pnpm generate:test <log-file> 05-agent-tools-menu
 *
 * @generated
 */

import { describe, it, expect, beforeAll, afterAll } from "vitest";
import { getFirstCustomerId, promoteToLeadGenerator, closeDbPool } from "./setup.js";
...
const SESSION_ID = `flow-test-05-agent-tools-menu-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
...
describe("05-agent-tools-menu - USSD Flow Test", () => {
  ...
  it("Turn 8: Input: \"0\"", async () => {
    ...
    // Cumulative USSD text: "2*1*CCDDE6B04*12345*1*1*0"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*12345*1*1*0`);
