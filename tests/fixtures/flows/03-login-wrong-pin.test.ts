 ...
 * - Session ID: rec-03-login-wrong-pin-1774608345411-9
 * - Phone: +260971230001
 * - Service Code: *2233#
 * - Recorded: 2026-03-27T10:45:50.739Z
 * - Turns: 9
 *
 ...
 *    pnpm test tests/flows/03-login-wrong-pin.test.ts  # Won't work
 *
 * Prerequisites:
 ...
 * 3. Run: pnpm generate:test <log-file> 03-login-wrong-pin
 *
 * @generated
 */
...
describe("03-login-wrong-pin - USSD Flow Test", () => {
  ...
  it("Turn 5: Input: \"99999\"", async () => {
    // Simulate realistic user interaction timing (2-second delay)
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Cumulative USSD text: "2*1*CCDDE6B04*99999"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*99999`);

    // Expected server response
    const expected = "CON 1 of 3 attempts - 2 left - account will be locked upon 3 incorrect attempts\n\nEnter your PIN:\n0. Back";

    // Assert response matches expected
    expect(response).toBe(expected);
  }, 10000); // 10 second timeout for this test

  it("Turn 6: Input: \"12345\"", async () => {
    // Simulate realistic user interaction timing (2-second delay)
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Cumulative USSD text: "2*1*CCDDE6B04*99999*12345"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*99999*12345`);
  ...
  it("Turn 7: Input: \"1\"", async () => {
    ...
    // Cumulative USSD text: "2*1*CCDDE6B04*99999*12345*1"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*99999*12345*1`);
  ...
  it("Turn 8: Input: \"1\"", async () => {
    ...
    // Cumulative USSD text: "2*1*CCDDE6B04*99999*12345*1*1"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*99999*12345*1*1`);
  ...
  it("Turn 9: Input: \"0\"", async () => {
    // Simulate realistic user interaction timing (2-second delay)
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Cumulative USSD text: "2*1*CCDDE6B04*99999*12345*1*1*0"
    // Send user input (USSD requires cumulative text)
    const response = await sendUssdRequest(`2*1*${CUSTOMER_ID}*99999*12345*1*1*0`);
