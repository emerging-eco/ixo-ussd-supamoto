...
if (sanitizedFlowName !== flowName) {
  console.log(
    `ℹ️  Flow name sanitized: "${flowName}" → "${sanitizedFlowName}"`
  );
}
...
try {
  ...
  console.log(
    `   pnpm vitest run --config vitest.flows.config.ts ${testFilePath}\n`
  );
  console.log(
    "   ⚠️  Note: Flow tests require the flow-specific config (vitest.flows.config.ts)"
  );
  console.log(
  ...
  console.log(
    "- Ensure the log file is a valid session log from pnpm test:interactive"
  );
