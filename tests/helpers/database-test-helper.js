...
function validateEnvironment() {
    try {
        ...
        throw new Error(`Database configuration validation failed: ${error instanceof Error ? error.message : String(error)}. ` +
 ...
 */
export async function initializeForTests() {
    if (isInitialized) {
...
export async function forceReset() {
    ...
    if (!ENV.IS_DEV_OR_TEST) {
        throw new Error("forceReset() is only allowed in test or development environment for safety. " +
