...
export class MockDatabaseService {
    ...
    async findUserByPhoneNumber(phoneNumber) {
        return (this.mockData.users.find(user => user.phoneNumber === phoneNumber) || null);
    }
...
export class MockIxoService {
    ...
    constructor() {
        ...
        this.mockData = dbService.mockData;
    }
    async getBalance(address) {
        const account = this.mockData.ixoAccounts.find(acc => acc.address === address);
    ...
    async sendTokens(fromAddress, toAddress, amount) {
        const fromAccount = this.mockData.ixoAccounts.find(acc => acc.address === fromAddress);
        ...
        let toAccount = this.mockData.ixoAccounts.find(acc => acc.address === toAddress);
        if (!toAccount) {
            toAccount = {
                address: toAddress,
                balance: amount,
...
export class MockUssdHandler {
    ...
    async handleUssdRequest(input, sessionId, phoneNumber, serviceCode) {
        try {
    ...
    async processUssdFlow(input, session, sessionId) {
    ...
    async handleWelcome(input, session, sessionId) {
        if (input === "") {
    ...
    async handleKnowMore(input, session, sessionId) {
        switch (input) {
            case "1":
    ...
    async handlePurchase(input, session, sessionId) {
        switch (input) {
            case "1":
    ...
    async handleTopUp(input, session) {
        const amount = parseFloat(input);
        if (isNaN(amount) || amount <= 0) {
            return "END Invalid amount. Please try again.";
        }
        // Mock top-up process
        const user = await this.dbService.findUserByPhoneNumber(session.phoneNumber);
        if (user) {
