...
export class ReplayTestHelper {
    ...
    loadFixture(filename) {
        try {
            ...
            console.error(`❌ Error loading fixture ${filename}:`, error instanceof Error ? error.message : String(error));
            throw error;
        }
    }
    /**
     ...
     */
    async replaySession(fixture, ussdHandler) {
        ...
        try {
            for (let i = 0; i < fixture.turns.length; i++) {
                ...
                try {
                    ...
                    const actualResponse = await ussdHandler(turn.textSent, fixture.sessionId, fixture.phoneNumber, fixture.serviceCode);
                    ...
                    console.log(`💥 Turn ${i + 1} threw error:`, error instanceof Error ? error.message : String(error));
                    const result = {
                        success: false,
                        turnIndex: i,
                        expected: turn.serverReply,
            ...
            console.log(`🎯 Success Rate: ${((passedTurns / fixture.turns.length) * 100).toFixed(1)}%`);
            ...
            console.error(`💥 Fatal error during replay:`, error instanceof Error ? error.message : String(error));
            throw error;
        }
    }
    /**
     ...
     */
    async replaySessionViaHTTP(fixture, serverUrl = "http://127.0.0.1:3005/api/ussd", timeout = 5000) {
        ...
        try {
            ...
            console.error(`💥 Fatal error during HTTP replay:`, error instanceof Error ? error.message : String(error));
            throw error;
        }
    }
    /**
    ...
    validateFixture(fixture) {
        if (!fixture.flowName || typeof fixture.flowName !== "string") {
            throw new Error("Invalid fixture: flowName is required and must be a string");
        }
        if (!fixture.timestamp || typeof fixture.timestamp !== "string") {
            throw new Error("Invalid fixture: timestamp is required and must be a string");
        }
        if (!fixture.sessionId || typeof fixture.sessionId !== "string") {
            throw new Error("Invalid fixture: sessionId is required and must be a string");
        }
        if (!fixture.phoneNumber || typeof fixture.phoneNumber !== "string") {
            throw new Error("Invalid fixture: phoneNumber is required and must be a string");
        }
        if (!fixture.serviceCode || typeof fixture.serviceCode !== "string") {
            throw new Error("Invalid fixture: serviceCode is required and must be a string");
        }
        ...
        for (let i = 0; i < fixture.turns.length; i++) {
            ...
            if (typeof turn.textSent !== "string") {
                throw new Error(`Invalid fixture: turn[${i}].textSent must be a string`);
            }
            if (typeof turn.serverReply !== "string") {
                throw new Error(`Invalid fixture: turn[${i}].serverReply must be a string`);
            }
            if (typeof turn.sessionId !== "string") {
                throw new Error(`Invalid fixture: turn[${i}].sessionId must be a string`);
            }
            if (typeof turn.timestamp !== "string") {
                throw new Error(`Invalid fixture: turn[${i}].timestamp must be a string`);
            }
            if (isNaN(Date.parse(turn.timestamp))) {
                throw new Error(`Invalid fixture: turn[${i}].timestamp must be a valid ISO 8601 date string`);
            }
        }
        // Validate main timestamp format
        if (isNaN(Date.parse(fixture.timestamp))) {
            throw new Error("Invalid fixture: timestamp must be a valid ISO 8601 date string");
        }
    }
}
//# sourceMappingURL=replay-test-helper.js.map
